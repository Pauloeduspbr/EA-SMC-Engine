...
input double         InpBOS_MinScore   = 0.30;            // BOS Min Score
...
input bool           InpOB_ReqHP       = true;            // OB Require High-Prob OB
...
input int            InpCH_Lookback    = 20;              // CHoCH Lookback (bars, very recent)
input double         InpCH_MinScore    = 0.45;            // CHoCH Min Score (high = strict)
input int            InpCH_MaxVisits   = 1;               // CHoCH Max Zone Visits
input bool           InpCH_ReqOTE      = true;            // CHoCH Require OTE Zone
...
input bool           InpFVG_Enable     = false;           // Enable FVG Fill
      ...
      if(dir != 0)
      {
         ...
         if(CanOpenTrade())
            ...
            Print("SIGNAL BLOCKED: ",
                  (!IsDayAllowed() ? "Day filter" :
                   (InpUseTimeFilter && !IsTimeAllowed()) ? "Time filter" :
                   (CountPositions() >= InpMaxPositions) ? "Max positions" :
                   (g_tradesToday >= InpMaxTradesDay) ? "Max trades/day" : "Unknown"));
      }
